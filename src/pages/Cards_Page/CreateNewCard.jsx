...
const CreateNewCard = () => {
  ...
  const handleSubmitOfUpdateCard = (event) => {
    ...
    fetch(
      `http://localhost:3306/api/update/card/user/${userId}/card/${cardId}`,
      {
        method: "PUT",
        mode: "cors",
        headers: new Headers({
  ...
  return (
    <form
      onSubmit={
        context.editCard
          ? handleSubmitOfUpdateCard
          : handleSubmitOfCreateNewCard
      }
    >
      <Layout
        title={context.editCard ? "Cards / Edit Cards" : "Cards / New Card"}
        extraControls={
          <span id="search-area">
                    ...
                    {colour.map((e, index) => (
                      <div className="box">
                        <input
                          ...
                          key={index}
                          onClick={context.handleCreateNewCardForm}
                          name="colour"
                          value={e.colour}
                          type="radio"
                        />
